...
- [Development guidelines](#development-guidelines)
    - [General Cheatscoin Core](#general-cheatscoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Cheatscoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play cheatscoins" on the test network, if you
...
Cheatscoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/cheatscoin-project/cheatscoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_cheatscoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_cheatscoin --log_level=test_suite
$ valgrind -v --leak-check=full src/cheatscoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_cheatscoin.coverage/index.html`.
```
...
To profile a running cheatscoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep cheatscoind` -- sleep 60
```
...
Cheatscoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Cheatscoin Core, where everyone uses
...
pay attention to for reviewers of Cheatscoin Core code.

General Cheatscoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Cheatscoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Cheatscoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Cheatscoin Core so that they can be integrated
...
file descriptors it will cause problems with Cheatscoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Cheatscoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cheatscoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cheatscoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Cheatscoin Core, the wallet was always
