 ...
 */
static const char* BITCOIN_PID_FILENAME = "cheatscoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("cheatscoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/cheatscoin-project/cheatscoin>";
    const std::string URL_WEBSITE = "<https://cheatscoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("cheatscoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if cheatscoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if cheatscoin is started while in a temporary directory.\n",
