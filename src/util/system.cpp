...
const char * const BITCOIN_CONF_FILENAME = "cheatscoin.conf";
...
#else
    const char* pszModule = "cheatscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cheatscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cheatscoin";
#else
    // Unix
    return pathRet / ".cheatscoin";
#endif
#endif
}
